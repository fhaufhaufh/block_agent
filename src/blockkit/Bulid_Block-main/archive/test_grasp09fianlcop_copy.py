                ...
                if conf > self.conf_threshold:
                    ...
                    if self.class_id == 2 and use_pca and hsv_min is not None and int(cls) == 2:
                        ...
                        if (x2 - x1) > 10 and (y2 - y1) > 10:
                            ...
                            if success:
                                # 使用 PCA 角度作为参考，解决 YOLO 角度的 180 度歧义
                                ref_angle = self.smoother.update(pca_angle)
                                standard_angle = ref_angle
                                # # YOLO 角度 (standard_angle) 只有 [0, 180)
                                # # 候选角度: YOLO, YOLO+180
                                # cand1 = standard_angle
                                # cand2 = (standard_angle + 180) % 360
                                
                                # # 计算与 PCA 参考角度的距离
                                # diff1 = abs(cand1 - ref_angle)
                                # diff1 = min(diff1, 360 - diff1)
                                
                                # diff2 = abs(cand2 - ref_angle)
                                # diff2 = min(diff2, 360 - diff2)
                                
                                # # 确保选择与PCA方向一致的角度 (正向)
                                # if diff2 < diff1:
                                #     standard_angle = cand2
                                # else:
                                #     standard_angle = cand1
                                    
                                print(f"Class 2 (Block 3). PCA: {pca_angle:.1f}, Ref: {ref_angle:.1f}, Final: {standard_angle:.1f}")
...
if __name__ == "__main__":
    ...
    # exit(0)
    place_pose = [[-0.062, -0.32692352748698496, 0.0165], [0, 0.7071068, 0, 0.7071068] ]
