...
class DetectionSystem:
    ...
    def process_frame_test(self, color_image, depth_frame, robot_position, robot_quaternion):
                ...
                if conf > self.conf_threshold:
                    ...
                    if depth_value > 0:
                        try:
                            ...
                            print(f"处理检测时出错: {e}")
        # cx, cy, standard_angle = 703, 340, 45
        # depth_value = self.get_average_depth(depth_frame, cx, cy, 132, 121)
        # # depth_value = 0.45
        # camera_coords = self.pixel2cam(cx, cy, depth_value)
        # base_coords = self.cam2base(
        #                         camera_coords[0], camera_coords[1], camera_coords[2],
        ...
        # grasp_quaternion = self.euler_to_quaternion(0, 90, 45)
        # print(f"base_coords: {base_coords}, grasp_quaternion: {grasp_quaternion}")
        # base_coords[2] = 0.1
        # detection = {
        #     'grasp_pose':{
        #         'position': base_coords,
        ...
        if detections:
            ...
            for det in detections_sorted:
                if det.get('class_id') == class_id:
                    target = det
                    break
            target['grasp_pose']['position'][2] = 0.15
            print("原始角度", target['obb_angle'])
            print("计算后的角度",target['image_angle'])
            if target is not None:
                self.robot.move_to_cart_pose([target['grasp_pose']['position'].tolist(), target['grasp_pose']['quaternion'].tolist()])  # 正上方
                target['grasp_pose']['position'][2] = 0.025
                self.robot.move_to_cart_pose([target['grasp_pose']['position'].tolist(), target['grasp_pose']['quaternion'].tolist()])  # 下去抓取
                self.robot.move_eef_pos([0.00]) # 闭合
                target['grasp_pose']['position'][2] = 0.15
                self.robot.move_to_cart_pose([target['grasp_pose']['position'].tolist(), target['grasp_pose']['quaternion'].tolist()])  # 抬起
            #     pose = target.get('grasp_pose', {})
            #     pos = pose.get('position')
            #     quat = pose.get('quaternion')

            #     # 将 numpy 数组转换为 list（如果需要）
            #     def to_list(x):
            #         try:
            #             return x.tolist()
            #         except Exception:
            ...
            #             except Exception:
            #                 return [float(x)] if not hasattr(x, '__iter__') else list(x)

            #     pos_list = to_list(pos) if pos is not None else None
            #     quat_list = to_list(quat) if quat is not None else None

            #     if pos_list is None or quat_list is None:
            #         print("目标姿态不完整，未执行移动:", pose)
            #     else:
            #         if hasattr(self, 'robot') and self.robot is not None and hasattr(self.robot, 'move_to_cart_pose'):
            #             try:
            #                 print(f"移动机器人到姿态: position={pos_list}, quaternion={quat_list}")
            #                 # 调用机械臂移动（传入列表形式）
            #                 self.robot.move_to_cart_pose([pos_list, quat_list])
            #             except Exception as e:
            #                 print(f"移动机器人时出错: {e}")
            #         else:
            #             print("机器人对象不可用或未连接，未执行移动。目标姿态:", pos_list, quat_list)
            # else:
            #     print("未找到 class_id==0 的检测目标。")
...
if __name__ == "__main__":
    ...
    # print(pose)
    place_pose = [[0.0007813723439017428, -0.33992352748698496, 0.03510353564706023], [0.015433232410532455, 0.6942487587734955, -0.0061318348804662555, 0.7195435197955868]]
