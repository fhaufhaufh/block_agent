...
class DetectionSystem:
    def __init__(self, config_path="config/camera.yaml"):
        ...
        print("检测系统初始化完成")
    def collect_template_routine(self, class_id=2):
        """
        收集模板模式：
        1. 识别目标
        2. 视觉伺服对齐中心
        ...
        while True:
            ...
            for attempt in range(5):
                ...
                target = None
                if detections:
                    valid = [d for d in detections if d['class_id'] == class_id]
                    if valid:
                        valid.sort(key=lambda x: x['dist_to_center'])
                        target = valid[0]
                ...
                dist = math.sqrt(target['dist_to_center'])
                if dist < 15: # 像素阈值
                    print("对齐完成")
                    align_success = True
                    break
                ...
                move_vec = R_base_end @ R_end_cam @ cam_offset
                
                target_pos = np.array(robot_pos) + move_vec
                if target_pos[2] > 0.25: target_pos[2] = 0.2
                
                self.robot.move_to_cart_pose([target_pos.tolist(), robot_quat])
                time.sleep(0.8)
            ...
            if align_success:
                ...
                raw_image = color_image.copy() # 备份原始图像用于模板
                
                # 检测 (color_image 会被绘制标注)
                processed_img, detections = self.process_frame_test(color_image, depth_frames, robot_pos, robot_quat)
                
                # 补全 dist_to_center 计算
                if detections:
                ...
                if target:
                    ...
                    w, h = target['size']
                    angle = target['obb_angle'] # 角度
                    
                    # 使用原始图像进行旋转矫正
                    M = cv2.getRotationMatrix2D((cx, cy), angle, 1.0)
                    h_img, w_img = raw_image.shape[:2]
                    rotated = cv2.warpAffine(raw_image, M, (w_img, h_img))
                    ...
                    if roi.size > 0:
                        ts = int(time.time())
                        path = f"templates/tpl_{class_id}_{ts}.jpg"
                        cv2.imwrite(path, roi)
                        print(f"模板已保存(纯净): {path}")
                    else:
                        print("ROI 无效")
                else:
                    print("对齐后丢失目标")
...
if __name__ == "__main__":
    ...
    # 进入模板收集模式 (针对 class_id=2, 即方块3)
    detector.collect_template_routine(class_id=2)
    ...
    detector.run_detection(class_id=2)  # 抓取第一个1
    ...
    place_pose[0][2] += 0.055
    place_pose[0][0] -= 0.065
    execute_place_sequence(place_pose)
