...
class HighPrecisionCalibrator:
    ...
    def run(self):
        ...
        for i, target in enumerate(targets):
            ...
            R_cam, t_cam = self.get_high_precision_marker_pose(sample_frames=30) # 采集45帧取平均
...
if __name__ == "__main__":
    ...
    try:
        R_res, t_res = calib.run()
        
        if R_res is not None:
            # Construct 4x4 matrix
            extrinsics = np.eye(4)
            extrinsics[:3, :3] = R_res
            extrinsics[:3, 3] = t_res.flatten()
            ...
            print(f"  camera_matrix: {calib.camera_matrix.tolist()}")
            print(f"  dist_coeffs: {calib.dist_coeffs.tolist()}")
