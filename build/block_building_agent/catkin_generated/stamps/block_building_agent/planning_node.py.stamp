...
class PlanningNode:
    def __init__(self):
        ...
        self.generated_action_script_path = rospy.get_param(
            ...
            "/tmp/generated_action_executor.py"
        )
    ...
    def _wait_for_world_blocks(self, timeout_sec):
        start = time.time()
        last_log = 0.0
        while not rospy.is_shutdown():
            if self.latest_world_blocks:
                return True
    ...
    def _run_action_planner_flow(self, final_task_sequence):
        ...
        got_world = self._wait_for_world_blocks(self.action_wait_timeout)
        if not got_world:
