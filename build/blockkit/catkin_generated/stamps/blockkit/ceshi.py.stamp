...
# 例如: [{'target_class': '1', 'target_place': [x1, y1, z1]}, ...]
task_queue = [
    {'target_class': '1', 'target_place': [0.5, 0.0, 0.1]},  # 示例位置，请根据实际情况修改
    {'target_class': '2', 'target_place': [0.5, 0.1, 0.1]},
    {'target_class': '3', 'target_place': [0.5, 0.2, 0.1]},
]
# 当前正在执行的任务
current_task = None
# 标记是否正在执行任务
...
def catch_and_place(obj_pose_in_base, arm_orientation_msg, target_place_pose):
    ...
    try:
        ...
        # --- Step 1: Move near the object (like original catch step 1) ---
        # Use MoveJ_P to move near the object (e.g., 7cm above or in front)
        # Adjust offset as needed based on your gripper approach strategy
        approach_offset = 0.07
        if not movejp_type([
            obj_pose_in_base[0] + approach_offset,
            obj_pose_in_base[1],
            obj_pose_in_base[2],
        ...
        # --- Step 2: Move to the object (like original catch step 2) ---
        # Use MoveL to precisely move to the object location
        if not movel_type([
            obj_pose_in_base[0],
            obj_pose_in_base[1],
            obj_pose_in_base[2],
        ...
        # --- Step 3: Close gripper (like original catch step 3) ---
        gripper_close()
        rospy.sleep(4) # Allow time for gripping

        print(f'  Moving to target place: {target_place_pose}')
        # --- NEW: Move to target placement location ---
        # Move slightly above the target first for safe travel
        place_above_offset = 0.1
        if not movejp_type([
            target_place_pose[0],
            target_place_pose[1],
            target_place_pose[2] + place_above_offset, # Go above target
            arm_orientation_msg.Pose.orientation.x,
            arm_orientation_msg.Pose.orientation.y,
            arm_orientation_msg.Pose.orientation.z,
            arm_orientation_msg.Pose.orientation.w
        ...
        # Now move down to the exact target location
        if not movel_type([
            target_place_pose[0],
            target_place_pose[1],
            target_place_pose[2],
        ...
        # --- Step 5: Return to ready pose (like original catch step 4) ---
